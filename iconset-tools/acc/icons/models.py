...
# this will store the  icons that are present  in both languages
words = {'en':{}, 'fr': {}}
icons = {'en':[], 'fr': []}


# TODO: some words (not many) may belong to multiple categories
# TODO: automatically generate thumbnails for "new" icons (I have thumbs for all French now)
def sqlite_process_icons_for_lang(lang = 'fr'):
    # TODO: multilanguage
    ...
    for meaning in IconMeaning.objects.all().filter(lang=lang):            
        ...
        # TODO: check if such a scenario is possible: two words are spelled the same, but their icon and part_of_speech are different

        # TODO: also we want to select the same icon over two languages for not to force to download them all!
        doub_key = word
        ...
        query = "%(word)s|%(word_ascii)s|%(part_of_speech)s|%(color_code)d|%(icon_path)s|%(lang)s|%(main_category_id)d|%(category_list)s" %  locals()
        
        if not words[lang].has_key(word):
            words[lang][word] = []
        words[lang][word].append( (icon_path, query) )

        icons[lang].append(icon_path)
...
def sqlite_gen_icons():
    sqlite_process_icons_for_lang('fr')
    sqlite_process_icons_for_lang('en')

    # get icons that are in both languages
    both_lang_icons = set(icons['fr']).intersection(set(icons['en']))
    queries_en = []
    queries_fr = []

    # simplification: just take english icons that are also contained in French!! as we have all french included
    for (word, icons_for_word) in words['en'].items():
        for (icon_path, query) in icons_for_word:
            if icon_path in both_lang_icons:
                # add this query (in both languages!) as it contains a word in both languages
                queries_en.append(query)
                del words['en'][word]
                break

    # for the ones left, select the first doublicate
    for (word, icons_for_word) in words['en'].items():
        (icon_path, query) = icons_for_word[0]
        queries_en.append(query)

    for (word, icons_for_word) in words['fr'].items():
        (icon_path, query) = icons_for_word[0]
        queries_fr.append(query)
