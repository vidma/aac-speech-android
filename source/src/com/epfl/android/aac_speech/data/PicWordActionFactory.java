...
import com.epfl.android.aac_speech.data.PicWordActionFactory.ActionDefinition;
import com.epfl.android.aac_speech.nlg.Pic2NLG;
import com.epfl.android.aac_speech.nlg.Pic2NLG.ActionType;

public class PicWordActionFactory {
	...
	private static final String TAG = "PicWordActionFactory";
	...
	protected static PicWordActionFactory INSTANCE = null;

	/*
	 * public static PicWordActionFactory getInstance() { if (INSTANCE == null)
	 * INSTANCE = new PicWordActionFactory(); return INSTANCE; }
	 */
	 ...
	 */
	public PicWordActionFactory(DBHelper db, String pref_gender, Resources res) {
		this.res = res;
	...
	PicWordAction getSpecialActionJe() {
		...
		return new PicWordAction(pronoun_I, je, ActionType.NOUN, R.drawable.je);
	 ...
	 */
	public String getSerializableString(PicWordAction icon) {
	 ...
	 * @return
	 */
	public String getSerialized(ArrayList<PicWordAction> phrase_list) {
		StringBuilder s = new StringBuilder();
		int i = 0;
		// update statistics for each individual icon
		for (PicWordAction word : phrase_list) {
	...
	public ArrayList<PicWordAction> createFromSerialized(String serialized) {
		// Log.d(TAG, "serialized" + serialized);
		ArrayList<PicWordAction> items = new ArrayList<PicWordAction>();
		for (String sItem : serialized.split(SEPARATOR_REGEXP)) {
			// Log.d(TAG, "sItem" + sItem);

			PicWordAction item = get(sItem);
			if (item != null)
				items.add(item);
		}
	 ...
	 * @return
	 */
	public PicWordAction get(String serialized) {
		...
		if (action_defs.containsKey(serialized)) {
			...
			return new PicWordAction(res.getString(act.titleResourceId),
