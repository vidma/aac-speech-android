...
//				if (!word.equals("ne") &&
	...
	protected Stack<NLGElement> createVerbGroup(PhraseElement phrase) {
		...
		if (progressive	&& (tenseValue != Tense.PAST || perfect || actualModal != null)) {
			NLGElement newFront = addProgressiveAuxiliary(frontVG, vgComponents, factory, phrase);
			if (frontVG != newFront) {
				frontVG = newFront;
				frontVG.setFeature(Feature.TENSE, tenseValue);
				insertClitics = false;
		...
		if ((tenseValue == Tense.PAST && (!progressive || perfect))
		...
		} else if (tenseValue == Tense.FUTURE && perfect) {
			auxReturn = addAuxiliary(frontVG, vgComponents, modal, Tense.FUTURE, phrase);
			frontVG = auxReturn.newFront;
		}
...
//				deVerb.setComplement(frontVG);
	...
	protected class AddAuxiliaryReturn {
		public NLGElement newFront = null, pastParticipleAvoir = null;
	...
	protected void pushFrontVerb(PhraseElement phrase,
		...
		if (Form.GERUND.equals(formValue)) {
			...
			vgComponents.push(frontVG);
				...
				|| formValue == Form.IMPERATIVE) || interrogative)
	...
	protected NLGElement checkIndirectObject(NLGElement element) {
		if (element instanceof NPPhraseSpec) {
			...
			PPPhraseSpec newElement = factory.createPrepositionPhrase("à", element);
