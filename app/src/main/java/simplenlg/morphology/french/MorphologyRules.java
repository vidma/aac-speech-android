...
//	}
	...
	public NLGElement doVerbMorphology(InflectedWordElement element,
		...
		if (Form.BARE_INFINITIVE.equals(formValue) || Form.INFINITIVE.equals(formValue) ) {
			...
			if (radical == null && baseWord != null) {
				...
				if (radical == null) radical = baseForm;
			}
			// build inflected form with radical or infinitive
	 ...
	 * @return the inflected word.
	 */
	protected String buildFutureVerb(String baseForm, NumberAgreement number,
			Person person) {
		String radical = getFutureConditionalRadical(baseForm);
		String suffix = "";
	 ...
	 */
	protected String getFutureConditionalRadical(String baseForm) {
		String radical = baseForm;
		int length = baseForm.length();
			...
			if (parent != null) {
				...
				if (!(function == DiscourseFunction.SUBJECT
					...
					if ( PhraseCategory.PREPOSITIONAL_PHRASE.equalTo(parent.getCategory())
						|| (person1or2 && parent.getFeature(Feature.FORM) == Form.IMPERATIVE
						...
						(grandParent != null &&
							(PhraseCategory.PREPOSITIONAL_PHRASE.equalTo(grandParent.getCategory())
								|| (person1or2 && grandParent.getFeature(Feature.FORM) == Form.IMPERATIVE
